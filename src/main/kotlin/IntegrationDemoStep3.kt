...
val integrateFunctional = { start: Double, end: Double, precision: Long,
                            f: (Double) -> Double ->
    val step = (end - start) / precision
    val xCoordinates = (0 until precision)
        .map { index -> start + index * step }
    val rectangleAreas = xCoordinates
        .map { xi -> step * f(xi) }
    rectangleAreas
        .sum()
}
...
fun main() {
    ...
    assertNearlyEq(
        integrateImperative(-15.5, 0.0, 1000, square),
